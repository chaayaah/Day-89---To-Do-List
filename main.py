...
class TaskPost(db.Model):
    ...
    assigned_to = db.Column(db.String(250), nullable=False)
...
@app.route('/')
def get_all_posts():
    result = db.session.execute(db.select(TaskPost))
...
def edit_post(post_id):
    post = db.get_or_404(TaskPost, post_id)
    ...
    if edit_form.validate_on_submit():
        post.task = edit_form.task.data
        post.assigned_to = edit_form.assigned_to.data
        post.due_date = edit_form.due_date.data
...
def delete_post(post_id):
    post_to_delete = db.get_or_404(TaskPost, post_id)
